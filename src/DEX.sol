...
contract Dex {
    ...
    function addLiquidity(
        ...
        require(
            liquidity > 0 && liquidity >= minLPAmount,
            "ERC20: transfer amount exceeds balance"
        );
        require(
            IERC20(token0).allowance(msg.sender, address(this)) >= amountA,
            "ERC20: insufficient allowance"
        );
        require(
            IERC20(token1).allowance(msg.sender, address(this)) >= amountB,
            "ERC20: insufficient allowance"
        );
        require(
            IERC20(token0).balanceOf(msg.sender) >= amountA,
            "ERC20: transfer amount exceeds balance"
        );
        require(
            IERC20(token1).balanceOf(msg.sender) >= amountB,
            "ERC20: transfer amount exceeds balance"
        );
    ...
    function swap(
        ...
        require(
            ...
            "Dex: INSUFFICIENT_INPUT_AMOUNT"
        );
        require(
            amountAIn == 0 || amountBIn == 0,
            "Dex: ONLY_ONE_INPUT_ALLOWED"
        );

        (address tokenIn, address tokenOut, uint256 amountIn) = amountAIn > 0
            ? (token0, token1, amountAIn)
            : (token1, token0, amountBIn);
        ...
        require(
            amountOut > 0 && amountOut >= minAmountOut,
            "Dex: INSUFFICIENT_OUTPUT_AMOUNT"
        );
