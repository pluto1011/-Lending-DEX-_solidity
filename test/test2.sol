...
contract Testx is Test {
    ...
    function testBorrowWithInsufficientCollateralFails() external {
        ...
        dreamOracle.setPrice(address(0x0), 1339 ether); //1339cusdc라는 뜻. ether는 걍 단위임 1e18대신 쓰는거
    ...
    function testBorrowMultipleWithInsufficientCollateralFails() external {
        supplyUSDCDepositUser1(); //1000000000...???cusdc
        supplySmallEtherDepositUser2(); //1이더

        dreamOracle.setPrice(address(0x0), 3000 ether); //3000cusdc로 변경.
    ...
    function testBorrowWithInSufficientCollateralAfterRepaymentFails() external {
        ...
        vm.startPrank(user2);
        {
            ...
            assertTrue(usdc.balanceOf(user2) == 2000 ether); //borrow 2번
            ...
            (success,) = address(lending).call(
                abi.encodeWithSelector(
                    DreamAcademyLending.repay.selector, //3999....정도의 collateral이 있나 봄. 이거 비율 맞지 50%까지밖에 못빌리나?
                    address(usdc), //borrow 후에 repay하는 테스트가 두개뿐임. 그러면?
                    1000 ether
                )
            );
            assertTrue(success);
            ...
            (success,) = address(lending).call(
                abi.encodeWithSelector(
                    DreamAcademyLending.borrow.selector,
                    address(usdc), //repay하면 수수료 뜯기나? 하루라도 더 지나면 수수료 내야 하네.
                    1000 ether
                )
            );
            assertFalse(success);
    ...
    function testWithdrawYieldSucceeds() external {
        ...
        vm.startPrank(user3);
        usdc.approve(address(lending), type(uint256).max);
        lending.deposit(address(usdc), 30000000 ether); //이더 공급
        vm.stopPrank();
        ...
        vm.startPrank(user2); //1이더 가지고 있음
        {
            (success,) = address(lending).call(
                abi.encodeWithSelector(
                    DreamAcademyLending.borrow.selector,
        ...
        (success,) = address(lending).call(
            abi.encodeWithSelector(
                DreamAcademyLending.withdraw.selector, //이자 구현이 실제로 출금으로 이어지는 건 얘 하나뿐임.
                address(usdc),
                30001605 ether //이만큼의 이자는 어디서 가져온 거지??? 빌려준 이자인가?
            )
        );
        assertTrue(success);
    ...
    function testExchangeRateChangeAfterUserBorrows() external {
        ...
        vm.prank(user3);
        assertTrue(lending.getAccruedSupplyAmount(address(usdc)) / 1e18 == 30000792); //user1이 1000000000을 deposit함.
    ...
    function testLiquidationHealthyLoanAfterPriorLiquidationFails() external {
        ...
        vm.startPrank(user3);
        {
            usdc.approve(address(lending), type(uint256).max); //2700??쯤.  -500 2200 1500
            (bool success,) = address(lending).call(
                abi.encodeWithSelector(DreamAcademyLending.liquidate.selector, user2, address(usdc), 500 ether)
            );
            assertTrue(success);
